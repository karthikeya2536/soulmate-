...
function GoldParticles({ count = 80 }: { count?: number }) {
  ...
  useFrame(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const y = positions[i * 3 + 1] + Math.sin(t * speeds[i] + x) * 0.3;
      const z = positions[i * 3 + 2];
      dummy.position.set(x, y, z);
      dummy.rotation.set(t * speeds[i] * 0.3, t * speeds[i] * 0.2, 0);
...
export function AmbientScene() {
  return (
    ...
    >
      ...
      <pointLight position={[5, 5, 5]} intensity={1.5} color="#D4A574" />
